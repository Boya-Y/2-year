...
function App() {
  ...
  useEffect(() => {
    ...
    const updateScene1 = (interactive = true) => {
        ...
        ctx.save(); ctx.beginPath(); ctx.arc(avatarX + avatarR, avatarY + avatarR, avatarR, 0, Math.PI * 2); ctx.clip(); const herImg = imagesRef.current.her; if (herImg && herImg.complete && herImg.naturalWidth > 0) { ctx.drawImage(herImg, avatarX, avatarY, avatarR*2, avatarR*2); } else { ctx.fillStyle = '#ffe4e1'; ctx.fillRect(avatarX, avatarY, avatarR*2, avatarR*2); ctx.fillStyle = COLORS.xhs_red; ctx.beginPath(); ctx.arc(avatarX + avatarR, avatarY + avatarR + 5, 12, 0, Math.PI*2); ctx.fill(); } ctx.restore();
        ...
        if (s.scene1Reply.choiceMade) {
            ...
            currentY += h1 + 20; { const meImg = imagesRef.current.me; if (meImg && meImg.complete && meImg.naturalWidth > 0) { ctx.save(); ctx.beginPath(); ctx.arc(LOGIC_W - 40, currentY + 20, 20, 0, Math.PI*2); ctx.clip(); ctx.drawImage(meImg, LOGIC_W - 60, currentY, 40, 40); ctx.restore(); } else { drawCircle(LOGIC_W - 40, currentY + 20, 20, COLORS.primary); } } const h2 = drawChatBubble(LOGIC_W - 70, currentY + 5, s.scene1Reply.userText, true);
            if (s.scene1Reply.timer > 80) {
            currentY += h2 + 20; ctx.save(); ctx.beginPath(); ctx.arc(avatarX + avatarR, currentY + 20, avatarR, 0, Math.PI * 2); ctx.clip(); const her2 = imagesRef.current.her; if (her2 && her2.complete && her2.naturalWidth > 0) { ctx.drawImage(her2, avatarX, currentY, avatarR*2, avatarR*2); } else { ctx.fillStyle = '#ffe4e1'; ctx.fillRect(avatarX, currentY, avatarR*2, avatarR*2); ctx.fillStyle = COLORS.xhs_red; ctx.beginPath(); ctx.arc(avatarX + avatarR, currentY + 25, 12, 0, Math.PI*2); ctx.fill(); } ctx.restore();
    ...
    const updateScene1Phone = () => {
        ...
        const infoY = 160; const rBig = 38; ctx.save(); ctx.beginPath(); ctx.arc(50, infoY, rBig, 0, Math.PI*2); ctx.clip(); const herImg = imagesRef.current.her; if (herImg && herImg.complete && herImg.naturalWidth > 0) { ctx.drawImage(herImg, 50 - rBig, infoY - rBig, rBig*2, rBig*2); } else { ctx.fillStyle = '#ffe4e1'; ctx.fillRect(50 - rBig, infoY - rBig, rBig*2, rBig*2); } ctx.restore();
